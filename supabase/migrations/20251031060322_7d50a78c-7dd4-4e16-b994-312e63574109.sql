-- Function to check if all recipients have signed and update letter status
...
-- Create trigger on signature_recipients table
DROP TRIGGER IF EXISTS trigger_update_letter_on_signature ON signature_recipients;
CREATE TRIGGER trigger_update_letter_on_signature
  AFTER UPDATE OF status ON signature_recipients
  FOR EACH ROW
  WHEN (NEW.status = 'signed' AND OLD.status != 'signed')
  EXECUTE FUNCTION update_letter_status_on_signature_completion();